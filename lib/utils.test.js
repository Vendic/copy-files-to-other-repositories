const { filterOutMissingBranches, filterOutFiles, getFilteredFilesList, getFileName, getAuthanticatedUrl} = require('./utils');
...
describe('getAuthanticatedUrl' , () => {
  it('It should return the oauth2 authentication url when a fine grained token is given', async () => {
    const fineGrainedToken = 'github_pat_98778799879787hjhkejhk89';
    const repoUrl = 'https://github.com/derberg/copy-files-to-other-repositories';
    const expectedResult = `https://oauth2:${fineGrainedToken}@github.com/derberg/copy-files-to-other-repositories.git`
    const actualResult = getAuthanticatedUrl(fineGrainedToken, repoUrl)

    expect(expectedResult).toEqual(actualResult);
  })

  it('It should return the non oauth2 authentication url when a legacy token is given', async () => {
    const legacyToken = '98778799879787hjhkejhk89';
    const repoUrl = 'https://github.com/derberg/copy-files-to-other-repositories';
    const expectedResult = `https://${legacyToken}@github.com/derberg/copy-files-to-other-repositories.git`
    const actualResult = getAuthanticatedUrl(legacyToken, repoUrl)
